...
export function ReviewPage({ questions, progress, onRecordAnswer, onToggleBookmark, onBack, initialMode = 'completed' }: Props) {
  ...
  // and the one after it got skipped. The counts below stay live; the list he
  // is working through does not move under him.
  const filteredQuestions = useMemo(() => {
    if (mode === 'completed') return questions.filter(q => progress.answers[q.id]);
    if (mode === 'incorrect') {
      return questions.filter(q => progress.answers[q.id] && !progress.answers[q.id].isCorrect);
    }
    return questions.filter(q => progress.bookmarkedQuestions.includes(q.id));
